...
public interface NewsRepository extends JpaRepository<News, Long> {
    // @Query("SELECT n FROM News n WHERE LOWER(n.title) LIKE LOWER(CONCAT('%',
    // :search, '%')) OR LOWER(n.content) LIKE LOWER(CONCAT('%', :search, '%'))")
    // Page<News> findByTitleOrContentContainingIgnoreCase(@Param("search") String
